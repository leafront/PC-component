	 ...
	 */
		...
		Popup.prototype={
			...
			openBtn:function(_this) {
				_this.wrap.css({'left': _this.popLeft, top: -_this.popHeight});
				this.oPen.on('click', function () {
					_this.wrap.show().animate({'left': _this.popLeft, top: _this.popTop},_this.options.showTimes);
			...
			scroll:function(_this){
				$(window).scroll(function(){
					if(_this.wrap.is(':visible')){
						...
						_this.wrap.animate({'left':_this.popLeft,top:_this.popTop},_this.options.moveTimes).dequeue();
					}
				});
			},
			resize:function(_this){
				$(window).resize(function(){
					if(_this.wrap.is(':visible')){
						...
						_this.popLeft=_this.winWidth/2-_this.popWidth/2+scrollLeft;
						_this.popTop=_this.winHeight/2-_this.popHeight/2+scrollTop;
						_this.wrap.show().animate({'left':_this.popLeft,top:_this.popTop},_this.options.moveTimes).dequeue();
					}
				});
			},
			closeBtn:function(_this){
				this.oClose.click(function(){
					...
					_this.wrap.css({'left': _this.popLeft, top: -_this.popHeight,'display':'none'});
				});
			},
			drag:function(_this){
