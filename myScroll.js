	...
	$.fn.myScroll=function(options){
		...
		return this.each(function(){
			...
			if(options.scrollType=='left'){
				...
				var oUlWidth=aLiWidth*aLi.length;
				oUl.css('width',oUlWidth);
				var isWH=oUlWidth;
			}else{
				var aLiHeight=aLi.eq(0).outerHeight(true)
			    var oUlHeight=aLiHeight*aLi.length;
			...
			if(options.prev!='' && options.next!=''){
				_this.find(options.prev).on(options.eventType,function(){
					options.iSpeed=options.arrowSpeed;
				});
				_this.find(options.next).on(options.eventType,function(){
					options.iSpeed=-options.arrowSpeed;
				});
			}
