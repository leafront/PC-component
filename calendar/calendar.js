...
Calendar.prototype = {
  ...
  specialDate: function() {
    ...
    var tomorrow = startMonth+(parseInt(startDay) + 1);
    var afterTomorrow = startMonth+(parseInt(startDay) + 2);
    var holiday = {
      [today]: '今日',
      [tomorrow]: '明天',
      [afterTomorrow]: '后天',
  ...
  getMonthsNum: function() {
    ...
    for (var i = 0; i <= this.monthsNum; i++) {
      var firstday = new Date(this.year,this.month,1).getDay(); //当月第一天Date资讯
      if (this.month == 12) {
        this.month = 0
        this.year += 1
      }
      monthsResult.push({
        ...
        month: this.month,
        firstday: firstday,
        rows: Math.ceil((this.getDaysInMonth(this.year,this.month) + firstday)/7),
